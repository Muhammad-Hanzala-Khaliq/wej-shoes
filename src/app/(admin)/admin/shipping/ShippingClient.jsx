...
export default function ShippingClient({ initialRules, initialTotal, initialPage, initialTotalPages }) {
  ...
  }, [initialRules, initialTotal, initialPage, initialTotalPages]);
    ...
    <div>
      ...
      {rules.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-100 p-12 text-center">
          ...
          <Button className="mt-4" onClick={openAddModal}>Add First Rule</Button>
        </div>
      ) : (
        <>
                ...
                <tbody>
                  {rules.map((rule) => (
                    <tr key={rule.id} className="border-b border-gray-50 hover:bg-gray-50">
                      <td className="px-4 py-3"><span className="font-medium text-gray-900">{rule.name}</span></td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${rule.type === "FLAT" ? "bg-blue-100 text-blue-800" : rule.type === "WEIGHT" ? "bg-purple-100 text-purple-800" : "bg-green-100 text-green-800"}`}>
                          {rule.type}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-gray-700">{formatAmount(rule)}</td>
                      <td className="px-4 py-3 text-gray-700">{rule.freeShippingThreshold ? `PKR ${rule.freeShippingThreshold.toLocaleString("en-PK")}` : "None"}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${rule.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-500"}`}>
                          {rule.isActive ? "Active" : "Inactive"}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => openEditModal(rule)} className="p-1.5 text-gray-400 hover:text-blue-600" title="Edit">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                          </button>
                          {deleteConfirm === rule.id ? (
                            <div className="flex items-center gap-1">
                              <button onClick={() => handleDelete(rule.id)} className="px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700">Yes</button>
                            ...
                            <button onClick={() => setDeleteConfirm(rule.id)} className="p-1.5 text-gray-400 hover:text-red-600" title="Delete">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
            <div className="border-b px-6 py-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold">{editingRule ? "Edit Rule" : "Add Rule"}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t">
                ...
                <Button type="submit" isLoading={isSaving}>{editingRule ? "Update" : "Add Rule"}</Button>
              </div>
            </form>
          </div>
        </div>
